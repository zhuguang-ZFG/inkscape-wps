...
class TestLegacyMainWindowParityRegression(unittest.TestCase):
    ...
    def test_legacy_main_window_table_context_menu_matches_core_actions(self) -> None:
        ...
        self.assertIn(
            'menu.addAction("合并选区单元格", self._table_editor.merge_selected_cells)',
            code,
        )
        self.assertIn(
            'menu.addAction("拆分当前合并", self._table_editor.split_current_merged_cell)',
            code,
        )
...
class TestFluentContextAndUndoRegression(unittest.TestCase):
    def test_slide_list_focus_refreshes_undo_state(self) -> None:
        ...
        self.assertIn(
            "if event.type() == QEvent.FocusIn and obj is "
            "self._presentation_editor.slide_list_widget():",
            code,
        )
    ...
    def test_project_save_and_open_include_render_modes(self) -> None:
        ...
        self.assertIn(
            'f"表格 {self._table_render_mode_label()}，'
            '演示 {self._slides_render_mode_label()}。"',
            code,
        )
    ...
    def test_office_export_success_messages_explain_scope(self) -> None:
        ...
        self.assertIn("def _import_success_tip(self, kind: str, target_mode: str) -> str:", code)
        ...
        self.assertIn("建议先检查表格尺寸、网格线与右侧预览，再导出或发送。", code)
        self.assertIn("Markdown 已按分节导入到演示，建议先检查每页内容与预览。", code)
        self.assertIn(
            "f\"DOCX 已生成：{Path(path).name}。{self._office_export_tip('DOCX')}\"",
            code,
        )
        self.assertIn(
            "f\"XLSX 已生成：{Path(path).name}。{self._office_export_tip('XLSX')}\"",
            code,
        )
        self.assertIn(
            "f\"PPTX 已生成：{Path(path).name}。{self._office_export_tip('PPTX')}\"",
            code,
        )
        self.assertIn(
            "f\"Markdown 已生成：{Path(path).name}。"
            "{self._office_export_tip('Markdown')}\"",
            code,
        )
        self.assertIn(
    ...
    def test_send_actions_report_missing_device_connection(self) -> None:
        ...
        self.assertIn('self._notify_info("断点续发", "当前没有可续发的断点程序。")', code)
        self.assertIn('self._notify_error("软复位失败", "请先连接设备。")', code)
        self.assertIn('self._notify_error("继续失败", "当前未连接设备。")', code)
        self.assertIn('self._notify_info("继续发送", "当前没有待续发的 M800 后程序。")', code)

    def test_resume_flows_keep_pending_program_visible_and_retriable(self) -> None:
        code = Path("inkscape_wps/ui/main_window_fluent.py").read_text(encoding="utf-8")
        self.assertIn(
            "def _set_pending_resume_program(self, lines: Optional[List[str]]) -> None:",
            code,
        )
        self.assertIn('self._set_job_status("等待继续", len(before), len(lines))', code)
        self.assertIn('self._log_event("发送", f"已发送到 M800，剩余 {len(after)} 行待继续")', code)
        self.assertIn('self._set_pending_resume_program(list(after[idx:]))', code)
        self.assertIn('self._set_job_status("继续中断", idx - 1, len(after))', code)
        self.assertIn('self._notify_warning(', code)
        self.assertIn('"继续中断"', code)
    ...
    def test_table_paths_append_optional_grid_gcode_paths(self) -> None:
        ...
        self.assertIn('if table_mode == "outline":', code)
    ...
    def test_slides_paths_support_outline_mode(self) -> None:
        code = Path("inkscape_wps/ui/main_window_fluent.py").read_text(encoding="utf-8")
        self.assertIn('slides_mode = str(', code)
        self.assertIn('if slides_mode == "outline":', code)
        self.assertIn(
            "return self._presentation_editor.to_outline_paths_all_slides("
            "mm_per_px_resolver=_mm_px)",
            code,
        )
    ...
    def test_help_and_device_pages_offer_preflight_and_diagnostics(self) -> None:
        ...
        self.assertIn('sv.addWidget(QLabel("发送方式"))', code)
        self.assertIn('sv.addWidget(QLabel("接收缓冲预算（RX）"))', code)
        self.assertIn('self._cb_g92 = CheckBox("程序开始时临时对零（G92）")', code)
        self.assertIn('self._cb_m30 = CheckBox("程序结束发送 M30（否则 M2）")', code)
        self.assertIn('gv.addWidget(QLabel("开始前附加指令"))', code)
        self.assertIn('gv.addWidget(QLabel("结束后附加指令"))', code)
    ...
    def test_fluent_diagnostics_use_structured_log_and_preflight_report(self) -> None:
        ...
        self.assertIn("def _workflow_guidance_payload(self) -> tuple[str, str, str, str]:", code)
        self.assertIn(
            "def _set_workflow_card_state(self, card: QFrame, title: QLabel, "
            "detail: QLabel) -> None:",
            code,
        )
        self.assertIn(
        ...
        slides_code = Path(
            "inkscape_wps/ui/presentation_editor_pyqt5.py"
        ...
        self.assertIn(
            "def _set_health_card_state(self, card: QFrame, title: QLabel, "
            "detail: QLabel) -> None:",
            code,
        )
        ...
        self.assertIn('self._home_workflow_title = QLabel("下一步建议")', code)
        self.assertIn('self._dev_workflow_title = QLabel("下一步建议")', code)
        self.assertIn('self._home_workflow_action_btn = PrimaryPushButton("继续下一步")', code)
        self.assertIn('self._dev_workflow_action_btn = PrimaryPushButton("继续下一步")', code)
