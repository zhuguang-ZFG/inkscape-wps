...
- **核心中的核心**：`G-code` 生成与可靠刻写管线优先级高于 UI 相似度；UI 的主要职责是帮助用户稳定地产生、预览、导出并发送正确的路径数据。
...
1. **先保证生成得对**：输入内容 → `LayoutLine` → `VectorPath` → 坐标变换 → `G-code` 的链路必须可验证、可解释、前后一致。
2. **再保证用户不误操作**：预览、状态栏、撤销/重做、右键菜单、导出入口必须围绕“避免把错误内容送进 `G-code`”设计。
3. **最后才是更像 WPS**：接近 WPS 的布局、命名与工作流是为了降低学习成本，不以 1:1 复刻 Office/WPS 为目标。
4. **如有冲突，核心优先**：凡是“更像 WPS”和“更稳定的刻写结果”冲突时，以核心链路正确性为准。
...
- **三组件（对齐 WPS 三件套形态，互不混在同一富文本里）**：
  ...
  - **演示**：`ui/presentation_editor.py` 中 `WpsPresentationEditor`（左侧幻灯片列表 + 右侧每页 `QTextEdit`）；合并多页路径时各页沿 Y 错开；生成布局时使用**离屏** `QTextEdit` 排版，避免反复改写可见编辑器。**Fluent 主窗**：演示格式条提供 **标题1 / 标题2 / 正文** 预设（字号、加粗、段前后距），与右键「样式」一致，写入各页 HTML，随工程保存。
...
- **文字 / 演示 / 表格单元格编辑**：**字体 / 字号**、**加粗 / 倾斜 / 下划线**、**段落对齐** 在 **文字**、**演示** 当前页为选区合并；在 **表格** 为**当前选中单元格**整格应用（支持矩形选区合并单元格：合并以锚点单元格存储内容，渲染/G-code 唯一排版，见 `table_editor` 提示）。单元格内以 HTML 存 `UserRole`，原地键盘编辑会按纯文本刷新格式。快捷键使用 `QKeySequence.StandardKey`：**macOS 上为 Cmd+B/I/U 与系统对齐快捷键**；**Windows 上为 Ctrl**；两端对齐在无 `AlignJustify` 的平台回退为 **Ctrl+J**。
...
- **兼容 WPS/Office 文件导入**（迁移期实现）：
  - `.docx` → 导入到「文字」（转为 `word_html`）。
    - 主要保留：段落基本文本、粗体/斜体/下划线、字体族与字号（迁移期简化）。
    - 丢失/不支持：复杂表格、对象嵌入、页眉页脚等复杂版式域。
  - `.xlsx` → 导入到「表格」（转为 `table_blob`，迁移期以纯文本单元格为主）。
    - 主要保留：单元格文本；列宽/行高为近似（来自工作表尺寸映射为统一 `cell_w_mm/cell_h_mm`）。
    - 丢失/不支持：公式计算结果（不重算）、条件格式、富文本样式（迁移期以纯文本为主）。
  - `.pptx` → 导入到「演示」（每页提取文本，转为 `slides`）。
    - 丢失/不支持：完整母版布局系统、动画/过渡、多媒体对象、复杂文本框层级；仅保留母版页眉/页脚占位符（迁移期，仅文本）以及每页文本内容。
...
| 导出入口 | 产物 | 备注（能力边界） |
|---|---|---|
| 工程文件 | `*.inkwps.json` | 保存文字 HTML、表格单元格 HTML、演示每页 HTML、sketch 等；用于后续继续编辑与一致刻写。 |
| 生成 / 导出 G-code | `.nc/.gcode/.tap/.txt`（文本） | 来自当前选中页（文字/表格/演示/手绘/插入矢量）统一映射为 `VectorPath → paths_to_gcode`。 |
| 导出可见预览 PNG | `.png` | 保存当前视口内可见画面（与缩放/平移一致），非全尺寸场景渲染。 |
| 导出为 DOCX | `.docx` | 段落/运行级粗斜下划线与字体族/字号（迁移期简化）。 |
| 导出为 XLSX | `.xlsx` | 单元格文本为主；列宽/行高由统一 `cell_w_mm/cell_h_mm` 估算写入。 |
| 导出为 PPTX | `.pptx` | 每页提取文本为主要内容（迁移期简化）；母版页眉/页脚占位符会被套用到每页文本。 |
| 导出为 Markdown | `.md` | 以纯文本为主（演示/文字各自转为纯文本）。 |
...
| 演示页富文本 | **已改进**：**演示** 各页以 **HTML**（`QTextEdit.toHtml()`）持久化，换页保留加粗/斜体/下划线/对齐等；离屏排版路径生成时同步 `setHtml`。**已支持（迁移期）**：母版页眉/页脚占位符（仅文本）套用到每页；“主题”按钮将当前字体/字号/对齐/段前后距套用到所有幻灯片正文；**修订模式**（Fluent 演示编辑区右键）：开启后删除键为删除线标记，预览与 G-code 及 PPTX 用纯文本导出均**不输出**删除线字符；可「接受修订」（物理删除）或「拒绝修订」（去掉删除线）。**仍注意**：无插入修订高亮、无 Word 式修订 XML；与 WPS 相比未实现完整主题母版、动画与对象；复杂粘贴自 Office 的 HTML 可能排版略有差异。 |
...
## 7. 快捷键一览（Fluent 主窗 · 当前实现）

下列以 **Windows / Linux** 为主书写 **Ctrl**；在 **macOS** 上，系统对 `QKeySequence.StandardKey`（如 **新建、保存、加粗**）会映射为 **Cmd**。**段落对齐**（左/中/右/两端）在 macOS 上绑定为 **Cmd+L / E / R / J**（与 **Ctrl+** 对应项等价），见 `_paragraph_align_shortcut`。

### 7.1 菜单栏已绑定（`main_window_fluent.py`）

| 范围 | 快捷键 | 说明 |
|------|--------|------|
| 文件 | **Ctrl+N / O / S**（另存为 **Ctrl+Shift+S**） | 新建、打开、保存、另存为 |
| 文件 | **Ctrl+Q** | 退出 |
| 编辑 | **Ctrl+Z / Y** | 撤销 / 重做（目标随当前子页与焦点变化，见下） |
| 编辑 | **Ctrl+B / I / U** | 加粗 / 倾斜 / 下划线（**文字**页为单线模式时会提示并尽量不切页；**表格 / 演示**作用于选区或当前格） |
| 编辑 | **Ctrl+L / E / R / J**（macOS：**Cmd+** 同上） | 左 / 中 / 右 / 两端对齐 |
| 编辑 | **Ctrl+F**、**Ctrl+H**（若平台提供 Replace） | 查找、替换：文字/演示（QTextEdit），表格（按行优先跨单元格，支持当前/全部替换） |
| 编辑 | **Ctrl+A** | 全选（随当前子页路由到单线 / 表格 / 演示） |

### 7.2 故意未绑定菜单快捷键（避免与单线编辑重复）

| 动作 | 说明 |
|------|------|
| **剪切 / 复制 / 粘贴** | **编辑**菜单中的这三项**不设**全局快捷键；**单线文字**由 `StrokeTextEditor` 在控件内处理 **Ctrl+X/C/V**，避免与菜单双重触发（尤其粘贴）。**表格 / 演示**内仍可用系统编辑快捷键或格式条剪贴板按钮。 |

### 7.3 单线文字编辑区（`StrokeTextEditor`）

在 **文字** 页且焦点在单线编辑器时，由控件内部处理（与菜单无重复绑定）：

**Ctrl+A / C / X / V / Z / Y**，方向键、**Home / End**、**Backspace / Delete**、**Enter**（换行）。

### 7.4 其他交互（无单一全局快捷键）

| 场景 | 操作 |
|------|------|
| 路径预览 | **滚轮**缩放；右键菜单：缩放、复制/导出图像等 |
| 幻灯片列表 | 拖拽排序；焦点在列表时 **Alt+↑ / Alt+↓** 上移/下移幻灯片 |
| 设备 / 串口 | 以界面按钮与菜单「设备」为主 |

### 7.5 撤销 / 重做语义（与快捷键配合）

| 子页 | 焦点或场景 | 撤销栈 |
|------|------------|--------|
| **文字** | 单线编辑区 | `StrokeTextEditor` 文档模型栈 |
| **演示** | 右侧 `QTextEdit` 正文 | `QTextDocument` 栈 |
| **演示** | 左侧幻灯片列表等 | **表格 / 演示 / 手绘** 快照 `QUndoStack` |
| **表格** | 表格网格 | 同上 **QUndoStack**（非单元格内嵌编辑器独立栈） |
...
- 新增或删减**用户可见能力**时，应同步更新本 SPEC 的 **§3～§5**；**快捷键**变更时同步 **§7**。
